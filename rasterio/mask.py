...
def raster_geometry_mask(dataset, shapes, all_touched=False, invert=False,
    ...
    output_size_divider: (int, int) (opt)
        Scales the width and height of the output image. Defaults to (1, 1).
        The output size will be divided by the value.
    ...
    try:
        ...
        output_size = (base_size[0] // output_size_divider[1],
                       base_size[1] // output_size_divider[0])
        mask = numpy.ones(shape=output_size, dtype="bool") * (not invert)
        return mask, dataset.transform * affine_scale, None

    if crop:
        original_transform = dataset.window_transform(window)
    ...
    transform = original_transform * affine_scale
    out_shape = (height // output_size_divider[1],
                 width  // output_size_divider[0])
...
def mask(dataset, shapes, all_touched=False, invert=False, nodata=None,
    ...
    output_size_divider: tuple of divider_x:int and devider_y:int
        Output image size gets divided by (divider_x, divider_y)
    ...
    shape_mask, transform, window = raster_geometry_mask(
        ...
        pad=pad, pad_width=pad_width, output_size_divider=output_size_divider)
